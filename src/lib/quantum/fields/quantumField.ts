...
import { getPrimeFactors, spectralFormFactor, berryPhase } from '../utils/mathUtils';
...
export class QuantumField implements IQuantumField {
  ...
  initialize(token: string): void {
    ...
    this.primeFactors = getPrimeFactors(Math.abs(hash) + 1);
    ...
    for (let k = 0; k < this.values.length; k++) {
      ...
      // Compute Berry phase
      const berryPhaseValue = berryPhase(this.primeFactors, x);
      ...
      for (let i = 0; i < this.primeFactors.length; i++) {
        for (let j = i; j < this.primeFactors.length; j++) {
          ...
          const factor = spectralFormFactor(
            this.primeFactors[i],
            this.primeFactors[j],
            tau
          );
      ...
      const phase = berryPhaseValue + spectralSum.phase();
